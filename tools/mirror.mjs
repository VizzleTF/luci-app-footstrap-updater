...
/* No whole-file @same-file pins in this repo: the theme pinned LICENSE twice because its
 * PKG_LICENSE_FILES resolves inside the package build dir; this package sets no PKG_LICENSE_FILES, so
 * one root LICENSE (for GitHub) is enough. */
const SAME_FILE = [];
...
const SEARCH = [
	/* the self-updater backend carries the gh/* mirror copies install.sh is pinned against — they
	 * cannot share a file (install.sh runs from `curl | sh` before any package exists) */
	'luci-app-footstrap-updater/root',
	'luci-app-footstrap-updater/htdocs/luci-static/resources',
	'luci-app-footstrap-updater/Makefile',
	'install.sh',
];
const EXT = /\.(js|sh)$|(^|\/)Makefile$/;
